...
logger = setup_logger("02_perf_yf_etf_retry")
...
def load_failed_tickers(error_file):
    ...
    try:
        ...
        if "ticker" not in df.columns:
            logger.warning("⚠️  Error file missing ticker column: %s", error_file)
            return []
        return sorted(set(df["ticker"].astype(str).str.strip()))
    except Exception as exc:
        logger.warning("⚠️  Failed reading error file %s: %s", error_file, exc)
        return []
...
async def run_retry(tickers, reference_date: date, batch_size: int, output_dir):
    ...
    repair_log = output_dir / "yf_etf_repair_log.csv"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Retry Yahoo Finance ETF NAV scraping.")
...
def main():
    ...
    error_file = output_dir / "yf_errors_etf.csv"
